...
from tensorflow.keras.layers import Flatten, Concatenate, Input, ReLU, Dropout
...
def cre_model():
    # first
    x = LSTM(units=16, return_sequences=False, activation="relu")(trn_in)
    x = Flatten()(x)
...
if __name__ == '__main__':
    ...
    time_steps = 15
    seed_ = 110
    seed_tensorflow_(seed=seed_)

    name = 'cz'

    fac = np.load(r'data/' + name + '_fac_LSTM.npy')[:, 20 - time_steps:, :]
    ...
    for kk in range(1):
        ...
        stop = EarlyStopping(patience=10, min_delta=10, mode='min')
